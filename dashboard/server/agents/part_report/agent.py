...
from .schema import PartReportInput, payload_size_bytes
...
class PartReportAgent:
    def __init__(self, *, prompt_dir: Path | None = None, cache_root: Path | None = None, version: str = DEFAULT_PROMPT_VERSION) -> None:
        ...
        self.cache_root = cache_root or (Path("report_cache") / "llm" / "part_report")
        self.version = version

    def _cache_path(self, cache_key: str, variant: str) -> Path:
        return Path(self.cache_root) / variant / f"{cache_key}.json"

    def run(self, input: PartReportInput | dict, *, variant: str, debug: bool, nocache: bool = False) -> Dict[str, Any]:
        try:
            payload = input if isinstance(input, PartReportInput) else PartReportInput(**input)
        ...
        rows_json = json.dumps([row.model_dump() for row in payload.rows], ensure_ascii=False, separators=(",", ":"))
        user_prompt = (
            prompts["user"]
            .replace("{part_name}", str(payload.part_name))
        ...
        try:
            ...
            if parsed is None:
                ...
                repaired = ensure_json_object_or_error(raw_text, repair_prompt=repair_prompt, llm_call_fn=_repair_call)
                parsed = repaired
                repair_count = 1
            ...
            parsed = {"error": "PART_REPORT_LLM_ERROR", "message": str(exc)}
