...
- compact 변환은 memos/webforms를 모두 제거하므로 프런트/LLM이 메모 필드를 기대하면 데이터 손실로 오해할 수 있다.
...
- `/api/performance/pl-progress-2026/summary`가 T/E 컬럼을 모두 포함하고 current month E 셀 클릭 시 `/performance/pl-progress-2026/deals`가 recognizedAmount desc 정렬로 반환되는지 확인한다.
