...
- **Compact 변환** (`json_compact.compact_won_groups_json`):
  ...
  - day1_teams는 `team` JSON을 배열로 정규화하며, 날짜 필드는 모두 YYYY-MM-DD로 맞춘다. memos/webforms/null/빈 배열·객체는 재귀적으로 제거한다.
...
- compact 변환은 memos/webforms를 항상 제거하고, Won 딜이 아닌 값은 summary에 반영하지 않는다. deal_defaults는 3건 이상·80% 이상 반복일 때만 설정된다.
...
- 동일 org_id로 `/api/orgs/{id}/won-groups-json-compact`를 호출해 schema_version, organization.summary, deal_defaults, memos/webforms 제거가 적용됐는지 확인한다.
