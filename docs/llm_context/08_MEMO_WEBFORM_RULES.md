...
- compact 변환(`json_compact.py`):
  - memos/webforms 필드를 모두 제거하고, 날짜는 YYYY-MM-DD로 정규화된다.
...
- compact 변환은 memos/webforms를 항상 제거해 LLM 입력에서 개인정보를 노출하지 않는다.
...
## Verification
 ...
 - `/api/orgs/{id}/won-groups-json-compact`에서 memos/webforms가 제거되고 날짜가 YYYY-MM-DD로 정규화되는지 확인한다.
...
## Refactor-Planning Notes (Facts Only)
 ...
 - 개인정보 최소화를 위해 compact에서 memos/webforms를 제거하지만, 원본 JSON에서는 owner 이름 등 식별 정보가 그대로 남는다.
