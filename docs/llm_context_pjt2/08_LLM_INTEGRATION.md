...
## Purpose
- 카운터파티 리스크 카드용 LLM 입력/프롬프트/캐시/폴백을 재구현할 수 있게 상세 규칙과 원문 프롬프트를 제공한다.
...
- 캐시: payload canonical JSON → SHA256(llm_input_hash). prompt_version 불일치 시 무효화. 파일 경로 `report_cache/llm/{as_of}/{db_hash}/{org}__{counterparty}.json`.
- 실패/미호출: 폴백(blocker 키워드+숫자 근거)으로 evidence/actions를 채우며 job은 SUCCESS_WITH_FALLBACK 취급.

## Invariants
- Payload 필드:
  - counterparty_key(orgId/orgName/counterpartyName), tier
  ...
  - top_deals_2026: 금액 기준 desc 상위 5(최대 10). **출처: D5 report row에 포함된 리스트**(deal_norm 재조회 없음). 필드: id/name/status/possibility/amount/start/end/contract/expected_close/course_id_exists.
...
- 프롬프트: `dashboard/server/prompts/{system_prompt,user_prompt,repair_prompt}.txt` (주석 `#` 무시, 파일 없거나 오류 시 기본 상수 사용).
- Prompt 버전/모델: PROMPT_VERSION="v1". LLM env로 모델/키를 설정, 미설정 시 폴백-only.
...
## Coupling Map
- 구현: `dashboard/server/counterparty_llm.py` (payload/hash/candidates/fallback/cache), `deal_normalizer.py`(LLM 병합).
- 룰 참조: `05_RULEBOOK_COUNTERPARTY_RISK.md` for gap/coverage/risk.
- 캐시 경로/DB 해시: `deal_normalizer.build_counterparty_risk_report` (db_hash=mtime sha256 16자).

## Edge Cases
- LLM 미연동 상태: run_llm_or_fallback가 폴백만 반환. 모델 연결 시 해당 함수 교체.
...
- 폴백 동작: LLM 호출을 강제 실패시켜도 evidence 3개/actions 2~3개가 채워지는지 확인(counterparty_llm.run_llm_or_fallback).
...
- regex 매칭(한국어/영문 혼용): BUDGET/APPROVAL_DELAY/DECISION_MAKER/COMPETITOR/NO_RESPONSE/PRICE_TERM/SCHEDULE_RESOURCE/LOW_PRIORITY/FIT_UNCLEAR (counterparty_llm.BLOCKER_REGEX).
...
- 프롬프트 교체: `dashboard/server/prompts/*.txt` 수정으로 가능(placeholder/스키마 규약 유지).
## Verification
