...
## Behavioral Contract
- 매일 08:00(Asia/Seoul) cron(APScheduler)으로 **offline→online** 순으로 리포트를 생성하며, DB가 안정 상태가 아니면 재시도 후 실패 시 최근 성공본을 유지한다.
- 캐시는 offline `report_cache/YYYY-MM-DD.json`, online `report_cache/counterparty-risk/online/YYYY-MM-DD.json`에 원자적 저장, status는 mode별 파일(status.json/status_online.json)에 last_run/last_success 기록. LLM 실패는 폴백으로 채우고 성공으로 간주할 수 있다.
...
- 캐시 파일: tmp write + fsync + rename(atomic) → `report_cache/{mode}/{as_of}.json`(offline는 루트).
- status 파일: mode별(status.json/status_online.json) last_run(result/code/msg/as_of/db_signature), last_success(as_of/db_signature/generated_at).
