...
## Behavioral Contract
- 규칙/파이프라인/LLM/스케줄러/API는 Python unittest로 커버하며, 프런트는 JS 스모크(test/org_tables_v2_frontend.test.js)로 최소 렌더를 확인한다.
- 동일 payload에서 LLM 캐시가 재사용되고, memo/딜 변경 시 llm_input_hash가 바뀌는지 확인한다.
- API/스케줄러는 cache miss→generate(force)→serve, 실패 시 last_success 폴백까지 end-to-end로 검증한다.

## Invariants
- 비온라인 판정: 과정포맷 3종(구독제/선택구매/포팅 온라인)만 is_online=1, 나머지/NULL은 is_nononline=1.
- deal_year 귀속: 수강시작일 우선 → 계약 체결일 → 수주 예정일.
...
- 단위 테스트:
  - `tests/test_deal_normalizer.py`: 온라인 분류, 연도 귀속, Convert 제외, 금액 파싱.
  - `tests/test_org_tier.py`: 티어 임계값, 삼성 제외.
  - `tests/test_counterparty_target.py`: baseline→target 산정, 예상/확정 필터.
  - `tests/test_counterparty_risk_rule.py`: coverage/gap/risk_level_rule, pipeline_zero, target=0, 미분류 플래그.
  - `tests/test_counterparty_llm.py`: fallback evidence/actions, llm_input_hash/canonical_json 안정성.
- LLM 캐시: counterparty_llm.compute_llm_input_hash 기반 `report_cache/llm/{as_of}/{db_hash}/{mode}/{org}__{counterparty}.json`.
- 프런트: org_tables_v2.html 메뉴 `counterparty-risk-daily`/`counterparty-risk-daily-online` 렌더·팀/파트 필터·LLM 모달이 정상 작동해야 한다.
