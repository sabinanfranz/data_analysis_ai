...
## Behavioral Contract
- **사이드바·라우팅**: `MENU_SECTIONS` 순서대로 사업부 퍼포먼스(2026 P&L, 2026 월별 체결액, 2026 Daily Report(WIP)) → 운영(2026 Target Board, 2026 카운터파티 DRI, 교육1/교육2 딜체크) → 분석(StatePath 24→25, 2025 체결액 순위, 조직/People/Deal 뷰어, 숨김: 2025 대기업 딜·People/업종별 매출) → 검수(개인별 세일즈맵 검수, 고객사 불일치). URL hash가 유효하지 않으면 `DEFAULT_MENU_ID="org-view"`로 이동한다.
...
- **딜체크·QC**:
  - `renderDealCheckScreen(team)`은 `/deal-check?team=edu1|edu2` 결과를 orgWon2025Total desc→createdAt asc→dealId asc로 테이블 렌더, memoCount=0이면 “메모 없음” 비활성 버튼을 보여준다.
...
- 메뉴 라벨·순서·섹션(`perf`→`ops`→`analysis`→`qa`)과 각 item id는 `MENU_SECTIONS` 정의와 일치해야 하며, 해시 미인식 시 `org-view`로 이동해야 한다.
...
- 딜체크 테이블은 memoCount 0일 때 버튼을 비활성화하고 orgWon2025Total desc→createdAt asc→dealId asc 정렬을 유지해야 한다. QC 상세 모달은 위배 룰이 없는 섹션을 숨기고 colgroup 폭을 고정해야 한다.
...
- `/deal-check?team=edu1|edu2` 결과가 orgWon2025Total desc→createdAt asc→dealId asc 정렬이고 memoCount 0일 때 버튼이 비활성화되는지 확인한다.
