...
- 행 클릭 시 상세: 선택 org/upper_org의 won-groups-json/people를 사용해 팀별 25 온라인·비온라인 합계/딜 목록, People 리스트(웹폼 버튼 포함)를 모달(xl)로 표시한다. 모달 상단에 25 담당자/팀&파트/DRI를 강조해 보여준다. 2025 소스는 `25 온라인 (비집계 소스)` 테이블과 `25 비온라인 (집계 소스)` 테이블 두 개로 분리돼 온라인이 먼저 표시되며, `26 비온라인 체결(집계 소스)` 테이블만 남는다(`26 비온라인 타겟` 제거). 각 표는 “이름” 바로 뒤에 “담당자” 컬럼을 두며, 금액은 `금액→예상 체결액` 폴백, 온라인 판정은 구독제/선택구매(온라인)/포팅 완전 일치, 계약체결일→수주예정일 순으로 연도·표시를 결정하고 성사가능성 “높음/확정”만 집계한다. 캐시 무효화 없음 문구를 상단에 노출.
