...
11. 상위 조직별 JSON(23/24/25 Won 기준)
   - 필터 바로 아래 JSON 카드가 추가된다. 선택한 회사에 대해 23/24/25년에 Won 딜이 있는 `소속 상위 조직`들만 추려, 상위 조직→팀별로 모든 딜(상태 무관)과 People/메모/웹폼 정보를 묶어 `/orgs/{id}/won-groups-json` 결과(JSON)를 표시한다.
   - JSON 복사 버튼으로 전체 결과를 클립보드에 복사할 수 있다. 로딩/오류/미선택 시 버튼은 비활성화된다. 조직을 바꾸면 즉시 이전 JSON 상태가 초기화된다. 캐시는 orgId별 Map에 저장된다.
