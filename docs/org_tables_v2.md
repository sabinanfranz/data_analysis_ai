...
## 프런트 전역 상태/캐시 구조
- 주요 상태(`state`): 활성 메뉴(`activeMenuId`), 선택 규모(`size`, 기본 `대기업`), 검색어, 조직 목록/선택, 선택 상위 조직(`selectedUpperOrg`), People/Deal 선택, 메모 목록, 모달 핸들, 랭킹 화면에서 넘어올 때 쓸 `pendingOrgId`/`pendingOrgFallback`.
...
7. 선택 초기화 버튼
   - 상위 조직/People/Deal 선택과 관련 메모/Deal 표를 모두 초기화하되, 조직/검색/규모 선택은 유지한다.
8. Breadcrumb
   - 조직/People/Deal 이름을 순서대로 표시. 선택이 없으면 `-`.
9. 상위 조직별 JSON(23/24/25 Won 기준)
   - JSON 컨테이너를 2개로 분리: `전체 JSON`(회사 기준 전체 상위 조직)과 `선택 상위 조직 JSON`(상위 조직 행을 클릭했을 때 해당 upper_org만 필터링).
   - 선택 전에는 `선택 상위 조직 JSON`이 빈 상태(“상위 조직을 선택하세요.”), 선택 후에만 필터링된 JSON을 표시/복사한다. 추가 API 호출 없이 프런트에서 `/orgs/{id}/won-groups-json` 결과를 필터링한다.
   - 각 컨테이너에 별도 복사 버튼이 있어 표시 중인 JSON만 복사한다. 로딩/오류/미선택 시 버튼은 비활성화된다. 조직을 바꾸면 두 컨테이너 모두 초기화된다. 캐시는 orgId별 Map에 저장된다.
